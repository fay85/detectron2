...
class EfficientNetBuilder:
    ...
    def __call__(self, in_chs, model_block_args):
        ...
        feature_idx = 0
        stages = []
        stages_for_feature_extraction = [1,2,4,6]
        # outer list of block_args defines the stacks ('stages' by some conventions)
        for stage_idx, stage_block_args in enumerate(model_block_args):
            self.stage_names['s'+str(stage_idx)]=stage_idx
            ...
            self._out_feature_strides.append(int(
                current_stride * np.prod([block_args['stride'] for block_args in stage_block_args])))
            #print('************  {} stage_block_args {}'.format(stage_idx,stage_block_args))
            ...
            for block_idx, block_args in enumerate(stage_block_args):
                ...
                if last_block and stage_idx in stages_for_feature_extraction:
                    feature_module = block.feature_module(extract_features)
                    if feature_module:
                        feature_module = 'blocks.{}.{}.'.format(stage_idx, block_idx) + feature_module
                    feature_channels = stage_block_args[block_idx]['out_chs']
                    #print('stage idx ! ',stage_idx)
                    self.features[stage_idx] = dict(
                        name='s'+str(stage_idx),
